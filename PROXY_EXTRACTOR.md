...
当前本地 WebUI 默认勾选 **显示真实凭据**，便于直接复制到代理工具。注意：文档、日志和终端输出仍不应打印真实密码或订阅 URL。
...
- **场景快捷卡片**：指纹浏览器、地区池、脚本/curl/Python、Android 全局代理。
- **自定义提取参数**：高级参数默认折叠，展开后可以手动选择区域、模式、格式和数量。
- **卡片结果区**：每条代理单独展示，支持复制本条和复制 curl。
- **批量文本区**：适合一次性复制或下载 TXT。
...
### 本地页面显示真实凭据
当前页面默认勾选“显示真实凭据”，用于本机快速导入代理平台、指纹浏览器或脚本。

### 可手动脱敏
如果取消“显示真实凭据”，输出会把密码显示为：
...
### 不会暴露订阅 URL
提取器不会返回原始订阅链接；最终汇报、日志和文档也不要打印真实订阅地址或真实密码。

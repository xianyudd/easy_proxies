  ...
  return <div className="page node-config-page settings-page">
    ...
    <section className="card">
      <div className="panel-header">
        <div><div className="panel-title">配置节点列表</div><div className="panel-subtitle">共 {rows.length} 条。订阅模式下手动节点会写入节点文件；免费源缓存节点不会在这里编辑。</div></div>
      </div>
      <DataTable headers={['名称', '来源', 'URI', '端口', '认证', '操作']} empty={nodesQuery.isLoading ? '加载中...' : nodesQuery.isError ? '接口失败，请先重试。' : '暂无配置节点'}>
        {rows.map((node, idx) => {
          ...
          return <tr key={nodeKey(node, idx)}>
            ...
            <td><span className="mono muted">{String(node.uri || '-')}</span></td>

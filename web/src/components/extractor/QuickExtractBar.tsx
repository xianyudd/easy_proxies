...
export function QuickExtractBar({ run, geoipEnabled = true, disabled = false }: {run: (patch: Partial<ExtractorParams>) => void; geoipEnabled?: boolean; disabled?: boolean}) {
  ...
  return <div className="quick-grid">
    ...
    <button className="quick" disabled={disabled || !geoipEnabled} title={geoipEnabled ? 'GeoIP /us/ URL' : 'GeoIP 地区池未启用'} onClick={() => !disabled && geoipEnabled && quick({ mode:'geoip', region:'us', format:'http_url', count:1, reveal:true })}><strong>美国地区池</strong><span>{geoipEnabled ? 'GeoIP /us/ URL' : 'GeoIP 未启用'}</span></button>
